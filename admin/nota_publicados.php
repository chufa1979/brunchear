...
if ((isset($_GET["eliminalinea"])) || (isset($_GET["eliminalinea_x"]))){
  $v = $_GET['vid'];
	...
	$query_rsc = "SELECT * FROM brunchear_nota WHERE id = '$v'";
  ...
  $deleteSQL = "DELETE FROM brunchear_nota WHERE id='$v'";
  mysql_select_db($database_cone, $cone);
  $Result1 = mysql_query($deleteSQL, $cone) or die(mysql_error());
	...
	mysql_select_db($database_cone, $cone);
	$query_rs1 = "SELECT * FROM brunchear_nota ORDER BY id DESC";
...
<div id="rutas">
  <div class="ruta">NOTAS&nbsp;&nbsp;&nbsp;&raquo;&nbsp;&nbsp;&nbsp;publicados</div>
</div>
<div class="main">
  <div id="contenido">
      ...
      <tr>
        ...
        <td class="content"><div class="listado-top">
          <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
              ...
              <td width="350" align="left">Titulo</td>
        ...
        <?php while($myrow = mysql_fetch_array($db->sql_result))   { ?>
          ...
          <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
              <td width="10" align="left">&nbsp;</td>
              <td width="350" align="left"><?php echo $myrow['titulo']; ?></td>
              <td align="left"><?php 
			  if ($myrow['categoria']==1) { echo "Brunch Tips";} 
			  if ($myrow['categoria']==2) { echo "Brunch-In";} 
			  if ($myrow['categoria']==3) { echo "Tendencias";} 
			  if ($myrow['categoria']==4) { echo "Circuito";} 
			  if ($myrow['categoria']==5) { echo "Brunch951";} 
			   ?></td>
              <td width="70" align="left"><a class="tooltips" href="nota_editar.php?vid=<?php echo $myrow['id']; ?><?php if (isset($_GET['offset'])) { echo "&offset=".$_GET['offset'];}?>"><img src="img/icono_editar.gif" alt="MODIFICAR" width="60" height="30" border="0" /><span>Modificar</span></a></td>
              <td width="60" align="right"><div class="listing" style="display:none" id="open_ajax_dialog_codediv1">
                  <div id="ajax_dialog<?php echo $myrow['id']; ?>">Dialog.alert({url: "nota_eliminar?vid=<?php echo $myrow['id']; ?>", options: {method: 'get'}},  
